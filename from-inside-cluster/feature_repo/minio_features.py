...
# FileSource pointing to the MinIO service via cross-namespace DNS
minio_source = FileSource(
    name="driver_stats_k8s_source",
    path="s3://test-bucket/feast/data/driver_stats.parquet",
    timestamp_field="event_timestamp",
    created_timestamp_column="created",
    description="Driver stats from MinIO S3 via cross-namespace DNS",
    s3_endpoint_override="http://minio-service.kubeflow.svc.cluster.local:9000"  # Cross-namespace service
)
...
california_source = FileSource(
    ...
    timestamp_field="event_timestamp",
    created_timestamp_column="created",
    description="California housing data from MinIO S3 via cross-namespace DNS",
    s3_endpoint_override="http://minio-service.kubeflow.svc.cluster.local:9000"  # Cross-namespace service
)
